...
const NAV_ITEMS = [
  ...
  { href: "/report-generation", label: "Report Generation", icon: FileSpreadsheet },
  { href: "/export-data", label: "Export Data", icon: FileOutput },
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const browserHasToken =
      ...
      (document.cookie.includes("akp_token=") ||
        (typeof window !== "undefined" && Boolean(window.localStorage.getItem("akp_token"))));
      ...
      <main className="relative flex min-h-screen flex-1 items-center justify-center overflow-hidden bg-slate-950 px-4 py-16">
        ...
        <div className="relative flex flex-col items-center gap-4">
          <div className="relative grid h-16 w-16 place-items-center">
            <span className="absolute h-16 w-16 animate-ping rounded-full border border-cyan-300/35" />
            <span className="absolute h-12 w-12 animate-ping rounded-full border border-indigo-300/45 [animation-delay:200ms]" />
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-indigo-300/30 border-t-cyan-300" />
          </div>
